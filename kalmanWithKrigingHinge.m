%% coarse hinge tidy function evaulation. with changes to include krigging the error of kalman filter
...
% initialization for without krigging
ytp1Wo        =zeros(size(devCenterd,1),size(nodeCoord,1));
yttWo         =zeros(size(ytp1));
VarYttWo      =zeros(size(ytp1));
VarYtp1Wo     =zeros(size(ytp1));
...
for i=numberCompleteMeasure+1:size(zt,1) % i for each part
    ...
    while ~isempty(nRegions)&&(nSnaps<=maxSnap)
        ...
        [ytt(i,:),VarYtt(i,:),ytp1(i,:),VarYtp1(i,:)]=getPredictionsNew(eigVec,att,ptt,H,V,R,covErr,...
                                                                hyp,nodeCoord,nodeCoord(regionIndex,:),tempZ,tempEigVec);
%         [yttWo(i,:),VarYttWo(i,:),ytp1Wo(i,:),VarYtp1Wo(i,:)]=getPredictions(eigVec,att,ptt,H,V,R,covErr);
%         [ytt(i,:),VarYtt(i,:),ytp1(i,:),VarYtp1(i,:)]=getPredictions(eigVec,att,ptt,H,V,R,covErr);
...
hold all;
for i=1:size(nodeIDoutRect,2) 
    
    regionIndex=find(nodeIDoutRect(:,i));%% finds selected nodes
