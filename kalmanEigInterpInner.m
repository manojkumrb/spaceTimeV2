%% Kalman with krigging the error term, including functions for interpolating eigen values trial for inner
% added approximate variance calculation
...
for i=numberCompleteMeasure+1:numberCompleteMeasure+2%size(zt,1) % i for each part
	...
	while ~isempty(nRegions)&&(nSnaps<=maxSnap)
		...
		rmseT(nSnaps)=sqrt(sum((diff).^2)/size(devAll,2));  % rmse without noisy input
	...
	% RMSE for (t+1) prediction (i.e the first measurement)
