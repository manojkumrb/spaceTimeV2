...
%                   same as BLL_SignBoot but using a VAR in levels for Ft
% 
% [CC,u] = BLL_SignBoot(Y,cd,q,r,p,h,nrest,rest,ndraws,horiz,nboot,maxrot,FP)
% 
% Outputs:
...
function [CC,u, BB] = BLL_SignBootVAR(Y,cd,q,r,p,h,nrest,rest,ndraws,horiz,nboot,maxrot,FP)
...
det=1;
[AA,v]=ML_VAR(F,p,1,h);                                                     % Estimating VAR on Factors   
...
for bb=1:nboot;        
    ...
    [~,vstar,~,CLstar]=ML_VAR(Fstar,p,1,h);                                             % Estimating VAR on Factors   
