% datain_real.m
% Read in Real Dataset for HOM project
...
xlsname = '/Users/mwatson/Dropbox/Hom_Factor/ddisk/data/hom_fac_1.xlsx';
% Read Data 
...
bpdata=0; 
for is = 1:ns_m; 
  if (includecode(is) ~= 0) & (sum(cat_include == floor(catcode(is)) ~= 0));          % Ignore series if includecode == 0
      x = datamat_m(:,is);
      if defcode(is) == 1;
          x = x./price_def;
...
% Form Panel Data set of transformed Data
for is = 1:ns_q; 
  if (includecode(is) ~= 0) & (sum(cat_include == floor(catcode(is)) ~= 0));
      x = datamat(:,is);
      if defcode(is) == 1;
          x = x./price_def;
          tmp = char(labvec_long(is));
