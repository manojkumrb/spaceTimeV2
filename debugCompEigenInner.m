...
load('optimisedHypParmInner.mat');
sigmaMes    =1E-4;
nu.Type     ='diag';
nu.StdDev   =1e-4;
devAll      =devCenterd;
nBasis		=40;%[2,3,4,5,6,7,8,9,10:5:60];         % set containing number of basis vectors
...
% 			[ytt(i,:),VarYtt(i,:),ytp1(i,:),VarYtp1(i,:),yttU(i,:),ytp1U(i,:),VarYttU(i,:)]=getPredictionsNewInnerDebug(interpEigVec,...
...
% function substittution


				eigVeckey=keyEigVec;
				nodeCoordinatesU=nodeCoord;
				nodeCoordinatesM=nodeCoord(iMnp,:);
				xTemp=nodeCoord(regionIndex,:);
				zTemp=tempZ;
				...
				diff1=zTemp-yTemp;
				[mpredicted,varPred] = gp(hyp, @infExact, m0, covFunc, likfunc, xTemp,diff1', nodeCoordinatesM );

				yttTemp=att'*eigVeckey'+mpredicted';   % taking transpose to maintain convention of deviation matrix(i.e. rep x nodes)
				VarYttTemp=diag(eigVeckey*ptt*eigVeckey'+(V+R))+varPred;
				ytp1Temp=att'*H'*eigVeckey';  % taking transpose to maintain convention of deviation matrix
				VarYtp1Temp=diag(eigVeckey*(H*ptt*H'+covErr)*eigVeckey'+(V+R));

				% prediction for all points on the mesh 
				[mpredictedU,varPredU] = gp(hyp, @infExact, m0, covFunc, likfunc, xTemp,diff1', nodeCoordinatesU );
				yttUTemp=att'*interpEigVec'+mpredictedU';   % taking transpose to maintain convention of deviation matrix(i.e. rep x nodes)
				VarYttUTemp=sum(interpEigVec.*(ptt*interpEigVec')',2)+varPredU; % computation to calculate diagonal values of product of 3 matrices
				% this variance neglects other kalman filter variance sources (V + R) as in line 54
				ytp1UTemp=att'*H'*interpEigVec';  % taking transpose to maintain convention of deviation matrix
				% VarYtp1U=diag(interpEigVec*(H*ptt*H')*interpEigVec');

				ytt(i,:)=yttTemp;
				VarYtt(i,:)=VarYttTemp;
				ytp1(i,:)=ytp1Temp;
				VarYtp1(i,:)=VarYtp1Temp;
				yttU(i,:)=yttUTemp;
				ytp1U(i,:)=ytp1UTemp;
				VarYttU(i,:)=VarYttUTemp;
