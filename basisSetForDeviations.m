...
fem=femInit();
fileName{1}=strcat(inpFiles,'\hinge.inp');%\3dTriaHinge.inp');%\top_hat.inp');%\halo.inp');%
...
% % %%% graphical selction
% % [ ~, ~,iMnp  ] = uniformKeyPointSelection( fem,idPart,25 );
% % [boundNodeID]=getBoundaryNodes(fem,domainID, 90);
...
Mnp=load('half_25_90.xyz');
iMnp=dsearchn(nodeCoord,Mnp);

devPatterns= load('hingeDevArSimLoc.mat');%flatPlateDevArLocators.mat');%
...
[U ,singVal,V]=svds(devCenterd,10);
...
eigVec=load('interEigVecs25_90_directionChange.mat');%interEigVecs.mat');%
