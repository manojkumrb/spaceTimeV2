...
fileName{1}=strcat(inpFiles,'\door_inner.inp');%\hinge.inp');%\Remesh_hinge.inp');%\top_hat.inp');%\halo.inp');%
...
% new option added a the default settings have changed
...
for k=1:nBatches
	...
	nodeIdNc=nodeIdDomain([63638,77129,86998,1231,74942,57604,49570,77502,...
		4094,66324,19369]);
	nodeIdClamps=nodeIdDomain([71134,85893,478,1106,72973,48943,76376]);
	nodeIdSlots=nodeIdDomain(22594);
	nodeIdPin=nodeIdDomain(3342);
	...
	dir1n(1:2:end,:)=dir1n(1:2:end,:).*-1;%[0,1,0]; % reversing alternate NC-clamp direction
...
% 		contourcmap('parula',0.25);
	...
	simData(k).FemDevDomain=femDevDomain;
	simData(k).DevNc=devNc;
	simData(k).DevClampa=devClamps;
...
% % dev=load('devAutoCorrInner_dense.mat');
% % dev=dev.femDevDomain;
% close all
% 
% devPatterns		=load('simAutoCorDevInnerBatchesCombined.mat');%  %hingeDevArSimLoc
% devPatterns		=devPatterns.simData;
% dev				=devPatterns(8).FemDevDomain;
% devNc			=devPatterns(8).DevNc(1:5,1:4);
% devClamps		=devPatterns(8).DevClamp(1:5,1:4);
% 
% for i=1:5
% 
...
% 'FontName'	 , 'Times New Roman');
% 
% print(h,'old','-dpng','-r400')
% export_fig(myTtile,'-png','-r400','-transparent');
% export_fig(myTtile,'-pdf','-transparent');
